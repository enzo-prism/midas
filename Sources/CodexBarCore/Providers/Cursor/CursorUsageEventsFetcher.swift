...
struct CursorUsageEventsFetcher: Sendable {
    ...
    /// deducts), distinct from the API-rate `tokenUsage.totalCents`. Returns `nil` when any
    /// valid event omits `chargedCents`, so callers never publish a partial lower-bound total.
        ...
        cacheRoot: URL? = nil) -> CostUsageDailyReport
    {
        ...
        for event in events {
            ...
            if usage.cost == .omitted {
                ...
                let catalog = resolvedCatalog ?? CostUsagePricing.modelsDevCatalog(cacheRoot: cacheRoot)
